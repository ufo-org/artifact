...
High and low water marks are configured when loading the library,
 we will set them to 2 and 1 megabytes respectively. This is very constrained,
 normally you would give UFOs several gigabytes to work with, but in this case we
 want to simulate a larger data set in limited time and space. It also shows how
 flexible the UFO system is even when the data is much larger than the memory
 available
...
To keep the runtimes down and notr be too taxing on your computer we will only
...
Both a good job with this because of loading large chunks
...
Since we are reading randomly from the vectors AltRep loads elements singly while
 UFOs still loads chunks. UFOs will have to load and unload chunks in and out of its
 cache to keep with in the 2MB memory limit, but that still results in a win.

## In-memory Arrays

Loading things from disk is perhaps not always a good comparison, however.
There are cases where the object would be held in memory
 but can't be because it is too large.
...
R will have the entire vbector in-memory while UFOs need to load
 and unload chunks to stay under the 2MB high-water mark for memory use
 while doing random reads accross 128MB.
 How does it compare to normal vector accesses?
